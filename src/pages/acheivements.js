...
const Achievements = () => {
  ...
  const handleAddField = () => {
    setAchievementFields([...achievementFields, { title: '', dateAchieve: '', description: '' }])
  }
  ...
  const handleSubmit = (event) => {
    ...
    axios.post('/api/form-data/achievements', achievementFields)
      .then(response => {
  ...
  return (
    <div className="App">
      ...
      <form onSubmit={handleSubmit}>
        {renderFields()}
        <button className="field" type="button" onClick={handleAddField}>Add Field</button>
        {achievementFields.length > 1 && <button type="button" onClick={handleRemoveField}>Remove Field</button>}
